...
export default function helpers(
  groups?: string | string[] | HelperRegistrationOptions,
  options?: HelperRegistrationOptions
): HandlebarsHelpers {
  let actualGroups: string[] | null = null;
  let actualOptions: HelperRegistrationOptions = {};
  ...
  if (actualGroups) {
    actualGroups.forEach((key: string) => {
      const group = lib[key as keyof typeof lib] as HelperGroup;
      if (group) {
        hbs.registerHelper(group);
      }
    });
  } else {
    forIn(lib, (group: HelperGroup) => {
      hbs.registerHelper(group);
    });
  }
 ...
 */
forIn(lib, (group: HelperGroup, key: string) => {
  defineProperty(helpers, key, (options?: HelperRegistrationOptions) => {
